...
void MemoryChecker::IgnoreChecks()
{
...
void MemoryChecker::ExpectAllocate(std::size_t iExpectedAllocatedBytes)
{
    this->m_ExpectedAllocatedBytes.push(iExpectedAllocatedBytes);
}

void MemoryChecker::ExpectDeallocate(std::size_t iExpectedDeallocatedBytes)
{
    this->m_ExpectedDeallocatedBytes.push(iExpectedDeallocatedBytes);
}

void MemoryChecker::CheckAllocate(std::size_t iAllocatedBytes)
{
...
void MemoryChecker::CheckDeallocate(std::size_t iDeallocatedBytes)
{
    if (true == this->m_IgnoreCheck)
    {
        return;
